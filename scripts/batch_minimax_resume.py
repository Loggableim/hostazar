#!/usr/bin/env python3
"""Resume MiniMax batch from image 39."""
    ...
    for attempt in range(3):
        ...
        if code == 1002:
            if attempt < 2:
                time.sleep(65)
                continue
            return None, "RPM limit exhausted"
        if code != 0:
...
START = 38  # 38 done in first run
remaining = all_slugs[START:]
print(f"=== MINIMAX RESUME: {len(remaining)}/{TOTAL} Bilder (ab {START+1}) ===")
...
print(f"\n=== FERTIG: ✅ {DONE} | ❌ {FAIL} ===")
with open(os.path.join(IMG_DIR, "_minimax_resume_log.txt"), "w") as f:
    f.write(f"MiniMax Resume {time.strftime('%Y-%m-%d %H:%M')}\n")
