#!/usr/bin/env python3
"""
Batch generate ALL 147 hostazar Hero-Bilder NEU via lokale Gen Queue (8283).
BRAND STYLE: furry editorial illustration (fester Style für hostazar)
"""
...
LOG_FILE = os.path.join(IMG_DIR, "_brand_batch_log.txt")
os.makedirs(IMG_DIR, exist_ok=True)

# === BRAND STYLE (IDENTISCH FÜR ALLE BILDER) ===
BRAND_STYLE = (
...
NEG = "photorealistic, painting, oil paint, watercolor, 3d render, photograph, realistic lighting, soft shading, gradient, blur, watermark, text, low quality, ugly, deformed, blurry"

# === PER-ARTICLE SUBJECT PROMPTS (149 Artikel) ===
SUBJECT_PROMPTS = {
    ...
    "cloudflare-pages-vercel-netlify-vergleich-2026": "anthropomorphic deployment bear comparing server options, CI/CD pipeline, static hosting logos",
    ...
    "cloudflare-waf-einrichten-2026": "anthropomorphic security wolf with WAF shield, bot filter, DDoS protection, firewall rules",
    ...
    "cuda-rocam-vulkan-lokale-llm-gpu-backend-vergleich-2026": "anthropomorphic GPU backend owl, CUDA nvidia, ROCm amd, Vulkan comparison, GPU architecture",
    "dayz-server-mieten-2026": "anthropomorphic survivor wolf in military gear, abandoned chernarus town, zombie horde distant",
    "ddos-schutz-gameserver-2026": "anthropomorphic DDoS shield bear, protection layers, attack graph, server firewall, cybersecurity",
    ...
    "docker-compose-vps-guide": "anthropomorphic docker whale with compose YAML, multi-container ship, reverse proxy, database, web server",
    ...
    "github-actions-ci-cd-pipeline-2026": "anthropomorphic github actions cat, YAML workflow pipeline, test automation, deploy arrows, runner bot",
    ...
    "hetzner-cloud-vs-dediziert-vergleich-2026": "anthropomorphic german engineer bear, hetzner data center, server comparison, price tags",
    ...
    "istio-service-mesh-2026": "anthropomorphic istio service mesh cat, envoy sidecar proxies, traffic routing, security shield, kubernetes",
    ...
    "mysql-mariadb-optimierung": "anthropomorphic database dolphin vs seahorse, database cylinders, query optimization, indexing gears",
    ...
    "quantisierte-modelle-gguf-awq-gptq-exl2-bitsandbytes-2026": "anthropomorphic quantization fox, GGUF AWQ GPTQ comparison, model compression, quality vs size chart",
    ...
    "steam-deck-vs-asus-rog-ally-vs-lenovo-legion-go-2026": "anthropomorphic handheld gamers: fox deck vs cat ally vs dog legion, three devices comparison",
    ...
    "text-generation-webui-oobabooga-einrichten-2026": "anthropomorphic oobabooga cat with Text Gen WebUI, model loading screen, GPU usage, chat interface",
    ...
    "vps-absichern-2026": "anthropomorphic security wolf with locked server, SSH key shield, firewall, fail2ban, protection layers",
...
def submit(slug):
    subject = SUBJECT_PROMPTS.get(slug, f"anthropomorphic {slug.replace('-', ' ')} scene, technology concept")
    ...
    for i in range(timeout // 2):
        ...
        try:
            ...
            if st == "done" and resp.get("output_path"):
                ...
                print(f"  ❌ {slug} failed: {resp.get('error','?')}")
                return False
        except:
            pass
...
# === MAIN ===
print("=== HOSTAZAR BRAND BATCH ===")
print(f"Style: {BRAND_STYLE[:60]}...")
print(f"Bilder: {len(SUBJECT_PROMPTS)}")

# Check queue
try:
    h = json.loads(urllib.request.urlopen(f"{QUEUE}/health", timeout=10).read())
    print(f"Queue: ready ({h.get('pending',0)} pending, {h.get('completed',0)} done)")
except:
    print("❌ Queue offline!")
    sys.exit(1)

# Batch submit all
slugs = sorted(SUBJECT_PROMPTS.keys())
jobs = []
for slug in slugs:
    jid = submit(slug)
    if jid:
...
print(f"\n=== RESULT ===")
print(f"✅ {len(ok)} erfolgreich")
print(f"❌ {len(fail)} fehlgeschlagen")
with open(LOG_FILE, "w") as f:
    f.write(f"Brand batch {time.strftime('%Y-%m-%d %H:%M')}\n")
    f.write(f"OK: {len(ok)}, FAIL: {len(fail)}\n")
    for s in ok: f.write(f"OK {s}\n")
    for s in fail: f.write(f"FAIL {s}\n")
